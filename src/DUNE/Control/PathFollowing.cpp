...
namespace DUNE
{
  namespace Control
  {
    ...
    void
    PathFollowing::onUpdateParameters(void)
    {
    ...
    void
    PathFollowing::onResourceInitialization(void)
    {
      requestDeactivation();
    }

    void
    PathFollowing::onResourceRelease(void)
    { }

    void
    PathFollowing::onEntityReservation(void)
    {
    ...
    void
    PathFollowing::consume(const IMC::Brake* brake)
    {
    ...
    void
    PathFollowing::consume(const IMC::NavigationUncertainty* nu)
    {
      m_ctm.nav_uncertainty = m_ctm.nav_unc_factor * std::sqrt(std::max(nu->x, nu->y));
    }

    void
    PathFollowing::consume(const IMC::Distance* dist)
    {
      if (isTrackingBottom())
    ...
    void
    PathFollowing::consume(const IMC::DesiredZ* zref)
    {
      if (isTrackingBottom())
    ...
    void
    PathFollowing::consume(const IMC::DesiredSpeed* dspeed)
    {
      if (isTrackingBottom())
    ...
    void
    PathFollowing::consume(const IMC::EstimatedState* es)
    {
    ...
    bool
    PathFollowing::navigationJumped(const IMC::EstimatedState* new_state,
    ...
    void
    PathFollowing::monitorAlongTrackError(void)
    {
    ...
    void
    PathFollowing::monitorCrossTrackError(void)
    {
    ...
    void
    PathFollowing::consume(const IMC::ControlLoops* cloops)
    {
    ...
    void
    PathFollowing::onActivation(void)
    {
      m_error = false;
      m_tracking = false;
      m_braking = false;
    ...
    void
    PathFollowing::onDeactivation(void)
    {
      if (m_ts.z_control)
    ...
    void
    PathFollowing::signalError(const std::string& msg)
    {
      m_error = true;
      err("%s", msg.c_str());
    ...
    void
    PathFollowing::updateEntityState(const std::string msg)
    {
      if (m_setup)
    ...
    void
    PathFollowing::configureControlLoops(uint8_t enable, uint32_t mask)
    {
      if (enable)
      {
    ...
    void
    PathFollowing::reportPathControlState(bool force = false)
    {
    ...
    void
    PathFollowing::loiter(const IMC::EstimatedState& state, const TrackingState& ts)
    {
    ...
    void
    PathFollowing::deactivateBottomTracker(void)
    {
      if (m_btrack == NULL)
        return;
